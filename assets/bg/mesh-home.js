(() => {
  ...
  if (!c) { c = document.createElement('canvas'); c.id = 'bg-net'; document.body.prepend(c); }
  const ctx = c.getContext('2d');

  function resize(){
    const w = Math.max(document.documentElement.clientWidth,  window.innerWidth  || 0);
    const h = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
    c.style.position='fixed'; c.style.inset='0'; c.style.pointerEvents='none'; c.style.zIndex='0';
  ...
  function draw(t=0){
    const time=t/1000, W=c.width/DPR, H=c.height/DPR;
