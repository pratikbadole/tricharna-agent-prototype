...
// Helper: parse proposed_action JSON at the end
function extractActionFrom(text) {
  // 1) JSON inside a fenced block
  const fence = text.match(/```[\s\S]*?```/);
  if (fence) {
    try { return JSON.parse(fence[0].replace(/```/g, '').trim()); } catch {}
  }
  // 2) Raw JSON as the whole message
  try { return JSON.parse(text.trim()); } catch {}
  return null;
}

exports.handler = streamifyResponse(async (event, stream) => {
  const END_MARK = "<|END_OF_STREAM|>"; // frontend looks for this
  const write = (s) => stream.write(typeof s === "string" ? s : String(s));
  ...
  // Fetch RAG context (best-effort; falls back automatically if embeddings missing)
  const ctx = await retrieveContext(message, 3);
  const ctxText = ctx.map(d => `# ${d.title}\n${d.content || ''}`).join("\n\n---\n\n");

  if (!OPENAI_API_KEY) {
    // mock streaming if key missing
    const mock = `🤖 (mock) I understood: "${message}".\nThis is a demo stream.\n\nDone.`;
    for (const ch of mock) { write(ch); await new Promise(r => setTimeout(r, 10)); }
...
Do NOT include any other fenced JSON.`;
  ...
  try {
    ...
    for await (const part of streamResp) {
      ...
      if (delta) {
        full += delta;
        write(delta); // <- stream token(s) to client
      }
    }
  } catch (err) {
    write(`\n\n[ERROR] ${err.message}`);
